...
namespace kickmsg
{
    ...
    void UdpMulticastBackend::open_sender()
    {
        ...
        if (sender_ >= 0 and not configure_sender(sender_))
        {
            ::close(sender_);
            sender_ = -1;
        }
    ...
    void UdpMulticastBackend::signal()
    {
        ...
        addr.sin_port        = htons(port_);
        uint8_t byte = 1;
        while (::sendto(sender_, &byte, sizeof(byte), 0,
