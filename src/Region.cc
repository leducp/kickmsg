...
namespace kickmsg
{
                                        ...
                                        uint16_t    creator_len)
    {
        ...
        // Built from the computed layout, never read back from shared memory.
        geometry_.sub_rings_offset  = sub_rings_offset;
        geometry_.sub_ring_stride   = ring_stride;
        geometry_.sub_ring_capacity = cfg.sub_ring_capacity;
        geometry_.sub_ring_mask     = cfg.sub_ring_capacity - 1;
        geometry_.pool_offset       = pool_offset;
        geometry_.slot_stride       = slot_stride;
        geometry_.pool_size         = cfg.pool_size;
        geometry_.slot_data_size    = cfg.max_payload_size;
        geometry_.max_subs          = cfg.max_subscribers;
        geometry_.commit_timeout_us = static_cast<uint64_t>(cfg.commit_timeout.count());
        ...
        {
            auto* slot = slot_at(base(), geometry_, i);
            slot->refcount = 0;
        ...
        {
            auto* ring = sub_ring_at(base(), geometry_, i);
    ...
    namespace
    {
        /// Header fields validated together, each read from shared memory exactly once.
        ...
        void validate_snapshot(HeaderSnapshot const& snapshot, std::size_t size)
        {
            Geometry const& geometry = snapshot.geometry;
            ...
            if (snapshot.channel_type != channel::PubSub and
                snapshot.channel_type != channel::Broadcast)
            {
            ...
            if (geometry.max_subs == 0 or
                geometry.pool_size == 0 or
                geometry.slot_data_size == 0 or
                geometry.sub_ring_capacity == 0 or
                geometry.slot_stride == 0 or
                geometry.sub_ring_stride == 0)
            {
                throw std::runtime_error(
            ...
            if (geometry.sub_rings_offset < sizeof(Header) or
                geometry.sub_rings_offset >= geometry.pool_offset or
                geometry.pool_offset >= snapshot.total_size)
            {
            ...
            if ((geometry.sub_rings_offset % CACHE_LINE) != 0 or
                (geometry.pool_offset      % CACHE_LINE) != 0 or
                (geometry.sub_ring_stride  % CACHE_LINE) != 0 or
                (geometry.slot_stride      % CACHE_LINE) != 0)
            {
            ...
            if (geometry.sub_ring_capacity > snapshot.total_size / sizeof(Entry))
            {
                throw std::runtime_error("Header geometry: sub_ring_capacity exceeds region");
            }
            std::size_t const min_ring_stride = sizeof(SubRingHeader) + geometry.sub_ring_capacity * sizeof(Entry);
            if (geometry.sub_ring_stride < min_ring_stride)
            {
            ...
            if (geometry.slot_data_size > snapshot.total_size - sizeof(SlotHeader))
            {
                throw std::runtime_error("Header geometry: slot_data_size exceeds region");
            }
            std::size_t const min_slot_stride = sizeof(SlotHeader) + geometry.slot_data_size;
            if (geometry.slot_stride < min_slot_stride)
            {
            ...
            std::size_t const rings_space = geometry.pool_offset - geometry.sub_rings_offset;
            if (geometry.max_subs > rings_space / geometry.sub_ring_stride)
            {
            ...
            std::size_t const pool_space = snapshot.total_size - geometry.pool_offset;
            if (geometry.pool_size > pool_space / geometry.slot_stride)
            {
                throw std::runtime_error("Header geometry: slot pool overflow total_size");
            }
        }

        // Validate a peer-written header and return the geometry pointer math may use.
        // Every field is copied once and only the copy is checked, so a peer rewriting
        // the header mid-validation cannot slip an unchecked value past it.
        Geometry validate_opened(void* address, std::size_t size)
        {
            if (size < sizeof(Header))
            {
                throw std::runtime_error("Buffer smaller than region Header");
            }
            auto const* h = static_cast<Header const*>(address);
            ...
            {
                throw VersionMismatch("Region version mismatch");
            }

            HeaderSnapshot snapshot{};
            snapshot.geometry.sub_rings_offset  = h->sub_rings_offset;
            snapshot.geometry.sub_ring_stride   = h->sub_ring_stride;
            snapshot.geometry.sub_ring_capacity = h->sub_ring_capacity;
            snapshot.geometry.sub_ring_mask     = h->sub_ring_mask;
            snapshot.geometry.pool_offset       = h->pool_offset;
            snapshot.geometry.slot_stride       = h->slot_stride;
            snapshot.geometry.pool_size         = h->pool_size;
            snapshot.geometry.slot_data_size    = h->slot_data_size;
            snapshot.geometry.max_subs          = h->max_subs;
            snapshot.geometry.commit_timeout_us = h->commit_timeout_us;
            snapshot.channel_type               = h->channel_type;
            snapshot.total_size                 = h->total_size;
            snapshot.creator_name_len           = h->creator_name_len;

            validate_snapshot(snapshot, size);
                                           ...
                                           char const* label)
    {
        ...
        region.size_ = size;
        region.name_ = label;
        region.geometry_ = validate_opened(region.base_, region.size_);
        return region;
    }
                                              ...
                                              char const* creator_name)
    {
        ...
        {
            SharedMemory shm;
            // A creator sizes the object before stamping it; smaller means it is not ready yet.
            if (shm.try_open(name) and shm.size() >= sizeof(Header))
            {
                auto* h = static_cast<Header*>(shm.address());
                if (h->magic.load(std::memory_order_acquire) == MAGIC and
                    h->version != VERSION)
                {
                    throw VersionMismatch(std::string{"Region version mismatch on "} + name +
                ...
                // SHM exists but magic not ready yet -- creator is still mid-init.
            }
    ...
    SharedRegion::HealthReport SharedRegion::diagnose()
    {
        ...
        for (uint64_t i = 0; i < geometry_.max_subs; ++i)
        {
            auto* ring    = sub_ring_at(b, geometry_, static_cast<uint32_t>(i));
            auto* entries = ring_entries(ring);
            uint64_t wp   = ring->write_pos.load(std::memory_order_acquire);
            uint64_t cap  = geometry_.sub_ring_capacity;
    ...
    std::size_t SharedRegion::repair_locked_entries()
    {
        ...
        kickmsg::sleep(microseconds{geometry_.commit_timeout_us});
    ...
    RegionStats SharedRegion::stats() const
    {
        ...
        RegionStats out{};
        out.pool_size    = geometry_.pool_size;
        out.total_steals = h->steal_count.load(std::memory_order_relaxed);
        out.rings.reserve(geometry_.max_subs);

        for (uint64_t i = 0; i < geometry_.max_subs; ++i)
        {
            // sub_ring_at needs a non-const base*, but the operation
            // is read-only -- const_cast is safe here.
            auto* ring = sub_ring_at(const_cast<void*>(b),
                                     geometry_, static_cast<uint32_t>(i));
            uint32_t packed = ring->state_flight.load(std::memory_order_acquire);
        ...
        uint64_t count = 0;
        uint64_t const limit = geometry_.pool_size;
        while (idx != INVALID_SLOT and count < limit)
        {
            if (idx >= geometry_.pool_size) break;
            auto* slot = slot_at(const_cast<void*>(b), geometry_, idx);
    ...
    RegionInfo SharedRegion::info() const
    {
        ...
        out.max_subs          = geometry_.max_subs;
        out.sub_ring_capacity = geometry_.sub_ring_capacity;
        out.pool_size         = geometry_.pool_size;
        out.max_payload_size  = geometry_.slot_data_size;
        out.commit_timeout_us = geometry_.commit_timeout_us;
        out.creator_pid       = h->creator_pid;
        out.created_at_ns     = h->created_at_ns;

        // Creator name tail: bytes written at offset sizeof(Header).
        // Re-bound the live length: a peer can rewrite it after validation.
        auto const* tail = static_cast<char const*>(base()) + sizeof(Header);
        std::size_t name_len = h->creator_name_len;
        std::size_t tail_len = geometry_.sub_rings_offset - sizeof(Header);
        if (name_len > tail_len)
        {
            name_len = tail_len;
        }
        out.creator_name.assign(tail, name_len);
        return out;
    }
    ...
    std::size_t SharedRegion::reclaim_orphaned_slots()
    {
        ...
        // Each claim owns exactly one reference, so under quiescence a slot's
        // refcount must equal the number of entries claiming it.
        std::vector<uint32_t> claims(geometry_.pool_size, 0);
        ...
        for (uint64_t i = 0; i < geometry_.max_subs; ++i)
        {
            ...
            // Every entry, not just the write_pos window: a stranded claim can
            // outlive the tenancy that wrote it. Locked and skip-marked entries
            // still own the slots named by their claims.
            for (uint64_t idx = 0; idx < geometry_.sub_ring_capacity; ++idx)
            {
                uint32_t biased = meta_slot_biased(entries[idx].meta.load(std::memory_order_acquire));
                if (biased != 0 and biased - 1 < geometry_.pool_size)
                {
                    ++claims[biased - 1];
                }
            }
        ...
        std::vector<bool> on_stack(geometry_.pool_size, false);
        uint64_t walked = 0;
        uint32_t idx32  = tagged_idx(h->free_top.load(std::memory_order_acquire));
        while (idx32 != INVALID_SLOT and idx32 < geometry_.pool_size
               and walked < geometry_.pool_size)
        {
            on_stack[idx32] = true;
            idx32 = slot_at(b, geometry_, idx32)->next_free.load(std::memory_order_relaxed);
            ++walked;
        }

        // Free unclaimed slots that are off the stack, and reset a claimed
        // slot's refcount to its claim count so a leaked reference cannot pin it.
        std::size_t reclaimed = 0;
        for (uint64_t idx = 0; idx < geometry_.pool_size; ++idx)
        {
            if (on_stack[idx])
            {
                continue;
            ...
            auto* slot = slot_at(b, geometry_, static_cast<uint32_t>(idx));
            if (claims[idx] != 0)
            {
                slot->refcount.store(claims[idx], std::memory_order_release);
                continue;
            }
