...
namespace kickmsg
{
    ...
    void Publisher::release_pending()
    {
        if (pending_slot_ != INVALID_SLOT)
        {
            ...
            auto* slot = slot_at(base_, geometry_, pending_slot_);
    ...
    AllocatedSlot::~AllocatedSlot()
    {
        if (valid())
        {
            publisher_->release_pending();
        }
    }

    AllocatedSlot::AllocatedSlot(AllocatedSlot&& other) noexcept
    ...
    AllocatedSlot& AllocatedSlot::operator=(AllocatedSlot&& other) noexcept
    {
        if (this != &other)
        {
            if (valid())
    ...
    bool AllocatedSlot::valid() const
    {
        return publisher_ != nullptr and not published_
           and publisher_->reservation_ == id_;
    }

    std::size_t AllocatedSlot::write(void const* src, std::size_t len)
    {
        if (not valid() or len > max_size_)
        {
            return 0;
        }
    ...
    std::size_t AllocatedSlot::publish(std::size_t len)
    {
        if (not valid())
        {
            return 0;
        }
        // Consumed either way: an oversized length recycles the slot.
        published_ = true;
        return publisher_->publish(len);
    }

    AllocatedSlot Publisher::allocate()
    {
        // Release any previously allocated but unpublished slot.
        release_pending();

        // Bump even if the pop fails: the previous slot is already back in the pool.
        ++reservation_;

        uint32_t slot_idx = treiber_pop(header_->free_top, base_, geometry_);
        ...
        return AllocatedSlot{*this, slot_data(slot), geometry_.slot_data_size, reservation_};
    }

    std::size_t Publisher::publish(std::size_t len)
    {
        // Reject oversized lengths before narrowing to uint32_t.
        if (len > geometry_.slot_data_size)
        {
            release_pending();
            return 0;
        }
        ...
        slot->refcount.store(static_cast<uint32_t>(geometry_.max_subs),
        ...
        for (uint32_t i = 0; i < geometry_.max_subs; ++i)
        {
            ...
            uint64_t idx  = pos & geometry_.sub_ring_mask;
    ...
    void Publisher::release_slot(uint32_t idx)
    {
        ...
        if (idx >= geometry_.pool_size)
        {
            return;
        }
        auto*    s    = slot_at(base_, geometry_, idx);
