...
- **has_waiter** (atomic uint32): which wake this subscriber is parked
  ...
  cleared by `clear_owner()` when its ring is reclaimed, which a
...
parked for -- is retracted by `clear_owner()` *before* that value is
...
the alternative: a `Waker` opens a descriptor, `wait_descriptor(sub)` hands
it to the caller's own `poll`/`epoll`/`kqueue` loop or to a `WaitSet`, and
the publisher makes it readable.

The backend is **injected into both ends by the caller** -- the
subscriber's through `Subscriber::attach()`, the publisher's through the
...
poll, so the wait caps at `wait_any()`'s poll cap and the loop re-arms on
its behalf.  A ring whose head is claimed but uncommitted fires no wake of any
kind, and caps the wait the same way.

Both caps are decided by the **arming pass itself**, not by a scan before
it.  A separate scan could observe `Armed` where `arm_wait()` goes on to
...
uses this copy because peers can still modify the shared header.
`SampleView` stores the slot pointer already resolved by its subscriber.
...
2. entry = entries[41 & mask]        Read the ring entry.
   ...
   |
   v
3. Read the slot index from the entry's claim word.
   |
...
Builds the set of slot indices named by ring entry claims -- whatever
the sequence word says, since a locked or skip-marked entry still owns
the slot its claim names -- walks the free stack to record
...
reclaim_orphaned_slots(region):
    ...
    for each ring i in [0, max_subs):
        for pos in [oldest_live, write_pos):
            claim = entries[pos].meta
            if claim names a slot:
                referenced.insert(slot named by claim)
...
```
Free -- CAS --> Claiming -- release-store --> Active
  ^                                           |
  |                 deregister: CAS Active -> Reclaiming
  |                 sweep: CAS generation even -> odd,
  |                        then store Reclaiming
  |                                           |
  +---- CAS Reclaiming -> Free <---------------+
```
