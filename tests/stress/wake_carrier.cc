...
bool run_wake_arm_race()
{
    std::printf("--- Wake backend: 40k arm/wait/disarm rounds against a live publisher ---\n");

    char const* shm_name = "/kickmsg_wake_arm_race";
    kickmsg::SharedMemory::unlink(shm_name);
    auto region = kickmsg::SharedRegion::create(
        shm_name, kickmsg::channel::PubSub, wake_cfg(shm_name), "wake_arm_race");

    UdpMulticastBackend backend{shm_name};
    Waker               waker{backend};
    Subscriber          sub(region);
    // No skip: the unit tests hard-fail without a carrier too, so a host that cannot
    // open one fails everywhere. Passing here would hide exactly what this run exists
    // to exercise.
    if (not sub.attach(waker))
    {
        std::printf("  FAIL: could not open the wake carrier\n");
        kickmsg::SharedMemory::unlink(shm_name);
        return false;
    }
    ...
    std::thread publisher([&]()
    {
        ...
        while (not stop.load(std::memory_order_acquire))
        {
            ...
            // Consume the request BEFORE announcing the send: the main thread arms the
            // next round the moment it sees `published`, and clearing `arm` after that
            // would wipe the new request and hang both sides.
    ...
    for (int round = 0; round < ROUNDS; ++round)
    {
        ...
        if (state == Subscriber::Wait::Armed)
        {
            signalled = readable(fd, 2ms);
        }
        ...
        if (state == Subscriber::Wait::Armed and not signalled)
        {
            signalled = readable(fd, 50ms);
        }
        sub.disarm_wait();
        ...
        if (state == Subscriber::Wait::Armed and not signalled
            and sub.wait_state() == Subscriber::Wait::Ready)
        {
            ++waited_without_wake;
        }
...
bool run_wake_arm_loop()
{
    ...
    while (received < COUNT and kickmsg::elapsed_time(start) < 60s)
    {
        nanoseconds budget = 5ms;
        switch (sub.arm_wait())
        {
            case Subscriber::Wait::Ready:  { budget = 0ns;   break; }
            case Subscriber::Wait::Poll:   { budget = 100us; break; }
            case Subscriber::Wait::Armed: { ++waits;        break; }
        }
        if (budget > 0ns)
        {
            readable(fd, budget);
...
bool run_wake_wait_any()
{
    ...
    auto a = kickmsg::SharedRegion::create(shm_a, kickmsg::channel::PubSub, wake_cfg(shm_a), "wake_any_a");
    auto b = kickmsg::SharedRegion::create(shm_b, kickmsg::channel::PubSub, wake_cfg(shm_b), "wake_any_b");
    ...
    if (not sub_a.attach(waker_a) or not sub_b.attach(waker_b))
    {
        std::printf("  FAIL: could not open the wake carrier\n");
        kickmsg::SharedMemory::unlink(shm_a);
        kickmsg::SharedMemory::unlink(shm_b);
    ...
    while ((got_a < COUNT or got_b < COUNT)
           and kickmsg::elapsed_time(start) < 60s)
    {
        ...
        if (sub_a.wait_state() != Subscriber::Wait::Ready
            and sub_b.wait_state() != Subscriber::Wait::Ready)
        {
            ++waits;
        }
        wait_any(subs, 2, 5ms);
