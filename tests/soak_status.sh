...
if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
    ...
    # A dead worker is only "finished" if it got far enough to write a
    # VERDICT; anything else died mid-run and must not look like success.
    verdict="$(grep -E 'VERDICT:' "$M" 2>/dev/null | tail -1 | sed -E 's/.*(VERDICT:.*)/\1/' || true)"
    if [ -n "$verdict" ]; then
        state="finished ($verdict)"
    else
        state="DIED EARLY -- no verdict; check $RUNDIR/stdout.log"
    fi
fi
...
san="$(grep '^SLICE ' "$M" 2>/dev/null | grep -oE ' san=[0-9]+' | grep -oE '[0-9]+' | awk '{s+=$1} END{print s+0}')"
hfail="$(grep '^SLICE ' "$M" 2>/dev/null | grep -oE 'hfail=[0-9]+' | grep -oE '[0-9]+' | awk '{s+=$1} END{print s+0}')"
...
echo "  slices   : $slices done   pass_scenarios=$pass   fail_scenarios=$fail   san_hits=$san   harness_fails=$hfail   fail_logs=$nfail"
echo "  per-profile:"
...
if [ "$nfail" -gt 0 ] || [ "$fail" -gt 0 ] || [ "$hfail" -gt 0 ]; then
