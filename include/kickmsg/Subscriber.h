...
namespace kickmsg
{
    ...
    public:
        ...
        private:
            ...
            SampleView(Header* hdr, SlotHeader* slot, uint32_t slot_idx, uint32_t len,
                       uint64_t ring_pos)
                : header_{hdr}
                , slot_{slot}
                , slot_idx_{slot_idx}
    ...
    private:
        ...
        void*                base_;
        /// Shared mutable state only (free_top, ring and entry atomics).
        /// Anything that drives pointer math comes from geom_; see Geometry.
        Header*              header_;
        Geometry             geom_;
        uint32_t             ring_idx_;
        uint64_t             start_pos_;
