...
namespace kickmsg
{
    ...
    /// findable by ADL, usually a hidden friend, which lets the descriptor stay private. Nothing derives from anything: a Subscriber, a socket from another
    ...
    ///
    /// A caller with its own event loop does not need this, and can take the descriptor
    /// from wait_descriptor() directly.
    class WaitSet
    {
    public:
