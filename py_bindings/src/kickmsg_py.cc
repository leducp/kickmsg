...
namespace kickmsg
{
    ...
    struct PyAllocatedSlot
    {
        ...
        PyAllocatedSlot(Publisher& p, void* data, std::size_t cap)
            : publisher{&p}, ptr{data}, max_size{cap}
            , token{p.reservation_id()}, published{false}
        {
        }

        /// Check that this handle still names the publisher's current reservation.
        bool current() const { return publisher->reservation_id() == token; }
    };
}
...
namespace kickmsg
{
    NB_MODULE(_native, m)
    {
        ...
        nb::class_<PyAllocatedSlot>(m, "AllocatedSlot",
            ...
            .def_prop_ro("valid",
                [](PyAllocatedSlot const& s) -> bool
                { return not s.published and s.current(); },
        ...
        nb::class_<Publisher>(m, "Publisher")
            ...
            .def("allocate",
                ...
                "invalid and refuses both publish() and memoryview().")
